...
/** Derived from rule "file : hdr row+ ;" */ file locals[int i=0]
   : hdr (rows += row[$hdr.text.split(",")]
   {$i++;})+
   {
...
/** Derived from rule "row : field (',' field)* '\r'? '\n' ;" */ row[String[] columns] returns[Map<String,String> values] locals[int col=0] @ init
   {
